...
<div class="row">
    <div class="col-md-12">
        ...
        {{ Breadcrumbs::render('micro-hub.documentVerificationData.index') }}
            ...
            <div class="grid-filter">
                <form method="get" action="">
                    <div class="row">
                        ...
                        <select class="form-control user-list" name="user">
                            <option value="">Select User</option>
                            @foreach($users as $user)
                                <option value="{{$user->id}}"  {{ ($user->id ==  $selectUser)?'selected': '' }}>{{$user->full_name}} ({{$user->id}})</option>
                            @endforeach
                        </select>
                    </div>
                        <div class=" col-sm-3">
...
<script>
    ...
    $(function () {
        appConfig.set('yajrabox.ajax', '{{ route('micro-hub.documentVerificationData.data') }}');
       ...
       /* appConfig.set('dt.order', [0, 'desc']);*/
        ...
        appConfig.set('yajrabox.ajax.data', function (data) {
            ...
            data.user = jQuery('[name=user]').val();
    ...
    $(document).on('change','.doc-status-change',function () {
        ...
        if(confirm_val)
        {
            $.ajax({
                type: "GET",
                url: "status/update/statusUpdate",
